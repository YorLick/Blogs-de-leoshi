...
router.get("/", async (req, res) => {
  try {
    const [rows] = await pool.query(
      'SELECT id, title, excerpt, author, DATE_FORMAT(created_at, "%d de %M, %Y") as created_date FROM posts ORDER BY created_at DESC',
    );
    res.json(rows);
  } catch (error) {
...
router.get("/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const [rows] = await pool.query(
      'SELECT id, title, content, author, DATE_FORMAT(created_at, "%d de %M, %Y") as created_date FROM posts WHERE id = ?',
      [id],
    );
    if (rows.length === 0) {
...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (result.affectedRows === 0) {
      return res
        .status(404)
        .json({ error: "Post no encontrado para actualizar." });
    }
