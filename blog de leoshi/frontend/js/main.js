document.addEventListener("DOMContentLoaded", () => {
  ...
  const DB = {
    ...
    getComments: (postId) =>
      JSON.parse(localStorage.getItem(`leoshi_comments_${postId}`) || "[]"),
    ...
    getChatMessages: () =>
      JSON.parse(localStorage.getItem("leoshi_chat_msgs") || "[]"),
    saveChatMessage: (msg) => {
    ...
    setSession: (user) =>
      localStorage.setItem("leoshi_session", JSON.stringify(user)),
  ...
  function checkSession() {
    ...
    if (blogTypeSubtitle) {
      blogTypeSubtitle.textContent =
        user && user.blogType ? user.blogType : "Blog Personal";
    }
  ...
  document.getElementById("form-register").addEventListener("submit", (e) => {
    ...
    if (DB.getUsers().length >= 50) {
      alert(
        "Lo sentimos, se ha alcanzado el límite máximo de 50 usuarios registrados en esta demo.",
      );
      return;
    }
  ...
  document.getElementById("form-recover").addEventListener("submit", (e) => {
    ...
    if (userIndex !== -1) {
      ...
      alert(
        "Contraseña actualizada correctamente. Por favor inicia sesión con tu nueva clave.",
      );
      openModal(modalLogin);
    } else {
  ...
  function loadEditProfile() {
    ...
    document.getElementById("edit-blog-type").value =
      user.blogType || "Blog Personal";
    ...
    document.getElementById("edit-profile-avatar-preview").src =
      user.avatar || "https://via.placeholder.com/100?text=U";
    ...
    if (installerArea) {
      ...
      installerArea.innerHTML =
        '<small style="display:block; width:100%; margin-bottom:5px; color:#777;">Instalador rápido (Click para añadir):</small>';
      ...
      popularSkills.forEach((skill) => {
        ...
        chip.onclick = () => {
          const current = skillsInput.value
            ? skillsInput.value.split(",").map((s) => s.trim())
            : [];
  ...
  document
    .getElementById("edit-avatar-file")
  ...
  document
    ...
    .addEventListener("submit", (e) => {
      ...
      if (fileInput.files && fileInput.files[0]) {
        ...
        document.getElementById("edit-profile-avatar-preview").src =
          croppedAvatar; // Feedback visual inmediato
  ...
  function openShareModal(post = null) {
    ...
    const textToShare = post
      ? `Mira este post: ${post.title}`
      : `¡Mira mi perfil en el Blog de Leoshi!`;
    ...
    document.getElementById("qr-username").textContent = post
      ? "este post"
      : user
        ? user.name
        : "invitado";
  ...
  function renderPosts(filterText = "") {
    ...
    if (posts.length === 0) {
      postsContainer.innerHTML =
        '<p style="text-align:center; width:100%; color:#777;">No hay artículos aún. ¡Sé el primero en escribir!</p>';
      return;
    }
    ...
    posts.forEach((post, index) => {
      ...
      if (post.type === "video") {
        ...
        const imgUrl =
          post.image || "https://via.placeholder.com/400x200?text=Blog+Leoshi";
      ...
      const tagsHtml = post.tags
        ? `<div class="post-tags-list">${post.tags
            .split(",")
            .map((t) => `<span class="post-tag">#${t.trim()}</span>`)
            .join("")}</div>`
        : "";
    ...
    list.innerHTML =
      comments
        .map((c, index) => {
          ...
          const userDisliked =
            currentUser && dislikes.includes(currentUser.name);
        ...
        .join("") ||
      '<p style="font-size:0.8rem; color:#999;">Sé el primero en comentar.</p>';
  }
  ...
  document
    .getElementById("form-create-post")
    .addEventListener("submit", (e) => {
      e.preventDefault();
      ...
      if (fileInput.files && fileInput.files[0]) {
        ...
        reader.onload = function (e) {
          const fileType = fileInput.files[0].type.startsWith("video")
            ? "video"
            : "image";
  ...
  window.viewUserProfile = (username) => {
    ...
    if (user) {
      ...
      document.getElementById("profile-view-avatar").src =
        user.avatar || "https://via.placeholder.com/100?text=U";
  ...
  function renderChatMessages() {
    ...
    chatMessagesArea.innerHTML = msgs
      .map((m) => {
        const isMine = currentUser && m.author === currentUser.name;
        // Añadir caso especial para el bot
        const authorUser =
          typeof BOT_USER !== "undefined" && m.author === BOT_USER.name
            ? BOT_USER
            : users.find((u) => u.name === m.author);
        const avatar =
          authorUser && authorUser.avatar
            ? authorUser.avatar
            : "https://via.placeholder.com/30?text=U";
        const content = m.image
          ? `<img src="${m.image}" class="chat-msg-img" alt="Imagen enviada" loading="lazy">`
          : m.text;
